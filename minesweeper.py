...
class State:  # state of the board as seen by a player
    ...
    def print_board(self):
      ...
      for r in range(10):
          for c in range(10):
              if self.mines[(r, c)] == "*":
                  print('*', end=' ')  # Print a '*' for mines
              elif self.mines[(r, c)] == '-':
                  print('.', end=' ')  # Print a '.' for unexplored squares
              else:
                  print(self.mines[(r, c)], end=' ')  # Print the number of adjacent mines
    ...
    def CheckConsistency(self, model, node, value):
        # Set the model's value for the current node
        model[node] = value

        # Iterate through each square in the model
        for square in model:
            if self.solution[square] == '*':
                if not model[square]:
                    return False
            else:
                # Calculate the number of mines around the square based on the model
    ...
    def update(self, node, frontier, node_list):  # this is called when we assign value to position (r, c)
        if self.solution[node] == "*":
            self.mines[node] = "*"
            self.game_over = True  # Set game over flag to True
            print(f"Mine uncovered at {node}. Game Over.")
            return frontier, node_list  # Early return as no further updates needed
        ...
        if len(squares) == self.mines[node] - mine_count:  # if number of available neighboring squares
            ...
            for i in range(0, len(squares)):
                self.mines[squares[i]] = "*"  # in this case all these squares will be mines
    ...
    while not frontier.empty() and len(state.positions) < 100:
        ...
        if frontier.empty():
            for _ in range(100):  # try up to 100 times to find a new start
                square = GetRandomSquare()
                if square not in state.positions and state.solution[square] != "*":
                    frontier.put((1, square))
...
#     with open("searchResults.txt", "a") as file:
#         file.write(f"Iteration {i}: Result = {result}, Time = {deltaTime} seconds\n")
#     print(i)
